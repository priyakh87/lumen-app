...
export function formatBookingDeleteMessage(booking) {
  if (!booking) return 'Deleted your upcoming booking.'
  const serviceName = booking.serviceName || 'booking'
  const dateLabel = booking.date ? new Date(`${booking.date}T00:00:00`).toLocaleDateString(undefined, {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  }) : 'your selected date'
  const timeLabel = booking.time || 'your selected time'
  return `Deleted your upcoming booking — ${serviceName}, ${dateLabel}, ${timeLabel}.`
}
